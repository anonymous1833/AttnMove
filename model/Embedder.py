...
def UserEmbedder(vocab_size = 10000, dim = 128):
	with tf.name_scope('UserEmbedding'):
		with tf.variable_scope('user_embedding',reuse=tf.AUTO_REUSE): #,reuse=True
			embedding = tf.get_variable('user_embeddings', [vocab_size, dim], initializer = tf.random_normal_initializer(mean=0, stddev=1)) #trainable=False
			print("user size:", vocab_size)
			return embedding
def DayEmbedder(vocab_size = 2, dim = 128):
	with tf.name_scope('DayEmbedding'):
		with tf.variable_scope('day_embedding',reuse=tf.AUTO_REUSE): #,reuse=True
			embedding = tf.get_variable('day_embeddings', [vocab_size, dim], initializer = tf.random_normal_initializer(mean=0, stddev=1)) #trainable=False
			print("day size:", vocab_size)
			return embedding